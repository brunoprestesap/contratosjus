...
export function UserForm({ user }: UserFormProps) {
  ...
  const passwordCheck = passwordValue.length > 0 ? validatePasswordStrength(passwordValue) : null;

  async function onSubmit(data: UserCreateInput | UserUpdateInput) {
    const result = isEditing ? await updateUser(user.id, data) : await createUser(data);
    ...
    <Card className="max-w-lg p-6">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {passwordCheck?.valid && <p className="text-sm text-green-600">Senha forte</p>}
        </div>
